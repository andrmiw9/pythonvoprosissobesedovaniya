...
[Ультра-полезный сайт по асинхронности в Python с миллиардом гайдов (на английском)](https://superfastpython.com/) 

[Гайды на любой случай жизни (на английском](https://realpython.com/)
...
Данные также могут быть **изменяемыми** и **неизменяемыми**
...
Основная разница между ними заключается в том, что список может быть изменен (мутабельный/изменяемый тип), а кортеж является неизменяемым (иммутабельным типом).
...
<h3><a name="8"> 8. Чем отличаются многопоточное и многопроцессное приложение?</a> </h3>

Многопоточное и многопроцессное (многопроцессорное) приложения отличаются друг от друга в том, как они используют ресурсы компьютера. 

В многопроцессных приложениях каждый процесс имеет свой собственный набор ресурсов, включая память, потоки, открытые файлы, сетевые соединения и другие системные ресурсы. 
Многопроцессорность в Python может быть достигнута с помощью библиотек `multiprocessing` (низкоуровневый доступ), `concurrent.futures` (высокоуровневый доступ) и `mpire` (самая быстрая). 
При этом в новом процессе запускается новый интерпертатор Python, со своим GIL. 

Пример
Допустим мы запускаем через `multiprocessing` вычисление каких-то функций из исходных данных на 4 процесса (при этом данные мы тоже делим на 4 части). Тогда OS (операционная система) запустит 4 процесса, каждый со своим интерпретатором и GIL, и каждый со своим потоком (с переменными и данными). После этого OS будет решать когда какому процессу выполнятся и на каком ядре процессора (но это уже не наша забота). Таким образом, мы будем *одновременно* на 4 ядрах (допустим) процессора совершать вычисления, что естественно ускорит их в 4 раза (в реальности чутка поменьше из-за накладных расходов на создание процессов - они довольно не маленькие). При этом, в каждом из 4 процессов будет работать свой GIL. Это и есть один из способ, как "обойти" GIL.
...
В Python (СPython реализация (дефолтная)) нет настоящей многопоточности для CPU-bound задач (задачи, нагружающие процессор) из-за GIL (смотрите далее). Однако для задач ввода-вывода (IO-bound, обращение к БД, сетевые запросы, чтение файла, etc.) реализация многопоточности в Python присутствует - за счет стандартной библиотеки `threading`. 

Современная альтернатива `threading` - это `asyncio`. Оно реализует всё в одном потоке, но с *event loop* - циклом задач, выполняемых последовательно. При этом не происходит переключения между потоками, что улучшает быстродействие. Таким образом, IO-bound задачи могут быть эффективно реализованы с помощью `asyncio`. Однако есть и обратная сторона медали - asyncio крайне желательно, чтобы весь код был асинхронным. Многие библиотеки попросту не поддерживают такой подход.

При правильном использовании все подходы могут ускорить выполнение программы и улучшить управляемость ею, однако многопоточное приложение может иметь проблемы с блокировками и условиями гонки при доступе к общим ресурсам. В многопроцессных приложениях каждый процесс защищен от других процессов и обеспечивает более высокую степень изоляции. `asyncio` лишена многопоточности, но достигает её за счет *event loop*.
...
А так выглядит многопроцессность

![img](https://i.imgur.com/1q0bNHf.png)

[Отличное видео, объясняющее многопроцессность](https://youtu.be/AZnGRKFUU0c?si=mz4ye_DGII112Hlx&t=633)
...
Оператор `*` вместе с оператором `**` также может применяться для распаковки значений. Оператор * используется для распаковки кортежей, списков, строк, множеств, а оператор ** - для распаковки словарей.

Подробнее про [упаковку и распаковку](https://metanit.com/python/tutorial/3.7.php)
...
Утилита `py_compile` (встроенная), позволит скомпилировать файл `.py` в байт-код и проверить его на наличие синтаксических ошибок без его запуска.

Используется `py_compile` довольно очевидно:
...
А вообще проверяют файл на ошибки без запуска любые линтеры. Самые популярные - `flake8` (минималистичный и легко настраиваемый), `pylint` (детальный, но медленный), `ruff` (современный, топ).



<h3><a name="15"> 15. Зачем в python используется ключевое слово self?</a> </h3>

В Python ключевое слово `self` используется для обращения к текущему объекту класса (инстансу). Оно передается как первый аргумент в методы класса и позволяет работать с атрибутами и методами объекта класса внутри этих методов.
...
Более глубокое погружение в декораторы - [Python декораторы на максималках](https://habr.com/ru/articles/750312/)



<h3><a name="17"> 17. Что может быть ключом в словаре?</a> </h3>

В Python ключом в словаре может быть любой *хэшируемый* объект, такой как число, строка или кортеж (если все его элементы хэшируемы). 
Например:

```python 
my_dict = {1: 'one', 'two': 2, (3, 4): 'three four'}
```
В этом примере ключами словаря являются число `1`, строка `'two'` и кортеж `(3, 4)`. Элементы 3 и 4 являются неизменяемыми и хэшируемыми int, поэтому и кортеж тоже хэшируемый.

Подробнее про хэшируемость и неизменяемость tuple: [Hashable, immutable](https://stackoverflow.com/questions/2671376/hashable-immutable)

Подробнее про хэшируемость в Python: [What is "hashable" in Python?](https://www.pythonmorsels.com/what-are-hashable-objects/)
...
Функция (дандер-метод, если точнее) `__init__` является конструктором (инициализатором) класса, и она вызывается автоматически при создании нового экземпляра класса. `__init__` используется для инициализации атрибутов, которые будут принадлежать объектам, создаваемым с помощью класса. 
...
Примечание: в Python есть настоящий конструктор, это дандер-метод __new__. Именно он *создает* новый обьект, а `init` его инициализирует.
Подробнее про конструктор и инициализатор в Python: [Python Class Constructors: Control Your Object Instantiation](https://realpython.com/python-class-constructor/#pythons-class-constructors-and-the-instantiation-process)
...
Это один из пунктов Common gotcha! в языке Python для всех людей, изучающих его.
...
`@property` декоратор используется для создания свойств объекта, которые можно получить и задать (методы-аксессоры - геттер и сеттер, см. [100. Что такое аксессоры...](#100)), но выглядят как обычные атрибуты объекта. Это позволяет управлять доступом к атрибутам объекта, установив условиями доступа и возможностью заложить дополнительную логику при чтении, установке или удалении атрибута. 
...
Примером использования асинхронного кода является библиотека `asyncio` в Python. Она реализует *кооперативную* асинхронную работу в Python (это когда программист или сама задача решает, когда прекратить своё выполнение и отдать контроль (выполнение) другой задаче).
...
Полное [руководство на русском по asyncio](https://habr.com/ru/companies/wunderfund/articles/700474/)

Для знающих английский - оригинал руководства: [Python Asyncio: The Complete Guide](https://superfastpython.com/python-asyncio/)

Более глубокое погружение в asyncio и обход неочевидных проблем: [asyncio: We Did It Wrong](https://www.roguelynn.com/words/asyncio-we-did-it-wrong/)
...
**Итератор** (Iterator) — это объект, который возвращает свои элементы по одному за раз и реализует тем самым протокол/шаблон проектирования (он встроен в Python). 

Итератор должен иметь дандер `__next__`, который возвращает следующий элемент и вызывает исключение `StopIteration`, когда элементы закончились (а ещё `__iter__` ). Итератор также может быть написан с помощью генераторов.
...
В этом примере мы создаем список и получаем из него итератор. Затем мы выводим элементы итератора с помощью функции `next()`, которая вызывает метод `__next__`
...
Еще один способ создания итераторов в Python — использование генераторов. Генератор — это функция, которая возвращает итерируемый объект в виде генератора (такой, как список или кортеж). Вместо того, чтобы возвращать все элементы сразу, генератор возвращает элементы по одному по мере необходимости.
...
Кстати, в пакете itertools есть `cycle` - бесконечный итератор.


<h3><a name="29"> 29. Что такое генератор? Чем отличается от итератора?</a> </h3>

**Генератор** - это функция, которая использует ключевое слово `yield` для возврата генератора. Любой генератор - это итератор (но не в обратную сторону). 

Также генератором может быть выражение, которое создает итератор на лету, с помощью `yield`.

Генератор может быть использован для создания последовательности значений, которые генерируются в момент обращения к ним, что позволяет эффективно использовать память и ускоряет выполнение программы. Короче, генератор основан на тех самых "ленивых" (отложенных) вычислениях (lazy evaluation).
...
Итераторы также могут быть созданы как классы, которые реализуют методы `__iter__` и
`__next__`, в то время как генераторы *уже несут в себе функционал итератора* и создаются при помощи функций, используя ключевое слово `yield`.
...
Ещё [статья с хорошими выводами-рекомендациями в конце](https://habr.com/ru/articles/560300/).

А ещё в генератор можно передавать параметры [как](https://skillbox.ru/media/code/generatory_python_chto_eto_takoe_i_zachem_oni_nuzhny/)
...
Вот пример декоратора на Python, который будет перехватывать ошибки и повторять функцию переданное кол-во раз:
```python
import functools
...
Для использования декоратора вам нужно добавить `@retry(max_retries=5)` перед определением функции, которую вы хотите обернуть:
```python
@retry(max_retries=5)
...
В этом примере функция `my_function` будет повторно вызываться до пяти раз в случае ошибки, до тех пор, пока она не выполнится успешно. Если после пяти попыток
функция не выполнится успешно, вы получите исключение.

Вы можете настроить параметр `max_retries`, чтобы изменить количество попыток.
...
Примечание: При наследовании используется особый порядок поиска нужного метода для вызова. Подробнее см. [51. Что такое MRO...](#51).
...
Ещё пару слов про хэш-коллизиции.
Есть разные стратегии их разрешения, но самые 2 популярные:
- Open Addressing - linear probing (по очереди на увеличение пытаемся засунуть ключ. Этот подход использует Питон, только с random probing, то есть пихаем в любое другое место и если свободно - то всё в шоколаде) 
- Separate Chaining - list'ы внутри одного ключа (попадаем на место, где уже есть значение - просто переделываем 1 значение в список значений и 2-ым пунктом туда добавляем текущее. Если снова попали сюда же - 3-им элементом и так далее)

В целом, при заполнении 70% доступных значений, рекомендуется расширять диапазон этих допустимых значений (в Python это происходит в dict при заполнении 2/3 возможных значений хэша)

Подробнее про [hash tables and hash collisions](https://adamgold.github.io/posts/python-hash-tables-under-the-hood/)

Подробнее про [внутреннее устройство set и dict](https://habr.com/ru/articles/830026/)

Если Вам по каким-то причинам интересны стратегии разрешения хэш-коллизий: [Collision Resolution Techniques](https://www.geeksforgeeks.org/collision-resolution-techniques/)
...
Если вам утверждают, что в Питоне используется передача значений по ссылке и для неизменяемых, и для изменяемых типов данных, просто приведите им этот пример:
```python
...
Подробнее по теме: [Что вы не знали о передаче параметров в функции](https://python-school.ru/blog/python/pass-by-assignment/)
...
- `ruff` - это новомодный инструмент, написанный на Rust. Самый быстрый по скорости работы. Можно настроить что хочешь, очень гибкий. В комплекте идет ещё и форматтер, но вот его уже настроить нельзя (тк он идейное продолжение black).
...
- `Black` - это инструмент для автоматического форматирования кода Python, который придерживается только одного стиля написания кода. Особо нельзя никак настроить.
...
На более глубоком уровне (старая статья, но суть не изменилась) [Как устроен GIL в Python](https://habr.com/ru/articles/84629/)
...
```
Примечание: интернирование работает не только для строк, но также для `int` в диапазоне от -5 до 256 вкл.



<h3><a name="61"> 61. Как упаковать бинарные зависимости?</a> </h3>

Современный метод - использовать uv. [Быстрый старт в мир Python окружений с uv](https://habr.com/ru/articles/875840/)
...
Подробнее [про wheels](https://realpython.com/python-wheels/)
...
Подробнее про [Виртуальные окружения в Python](https://semakin.dev/2020/04/python_virtualenv/)
...
- `uv` - это новомодный менеджер пакетов. Использует pyproject.toml и uv.lock. Очень шустро работает, быстрее pip-а. Легко настраивается.
- `poetry` - довольно молодой менеджер пакетов, предназначенный для замены в некоторой степени pip и virtualenv. В целом неплохой выбор, однако имеются пару проблем, которые разработчики много лет не исправляют.
- `pipenv` - обьединяет pip и virtualenv, управление через Pipfile. Причин использовать, относительно uv и poetry я не нашел.
...
- `conda` - это менеджер пакетов и среда управления, который позволяет управлять пакетами и зависимостями для проектов на Python, а также для других языков программирования и платформ. Используется в основном для научных вычислений.
...
Примечание: ещё есть классические С-подобные массивы, доступ через `array.array`. 
А также советую всё прогонять на бенчмарках, ведь что окажется более быстрым и подходящим в вашей конкретной задаче - а хрен его знает.
...
Однострочники — это вообще очень приятная питонячая радость, есть даже целые книги (рекомендую, легко гуглится `Кристиан Майер однострочники в Python filetype:pdf`). Однако не перебарщивайте - длинные однострочники крайне тяжело воспринимаются и читаются.
...
`itertools.cycle` - бесконечный итератор
...
Для работы с транзитивными зависимостями можно использовать систему управления зависимостями, например, `uv`, `pipenv`, `poetry` или `pip`. Эти системы позволяют устанавливать зависимости и их транзитивные зависимости, а также контролировать версии зависимостей. Например, при использовании `pipenv` / `uv` для установки и работы с зависимостями можно использовать следующие команды:

`pipenv install <имя пакета>` / `uv add <package_name>`
Эта команда установит пакет и его транзитивные зависимости и создаст файл Pipfile/pyproject.toml с перечнем зависимостей и версиями.

`pipenv shell`. 
Эта команда позволит активировать виртуальное окружение, в котором установлены зависимости.

`pipenv install --dev <имя пакета>` / `uv add --dev <package_name>`
Эта команда установит пакет в качестве зависимости разработки.

`pipenv uninstall <имя пакета>` / `uv remove <package_name>`
Эта команда удалит пакет и его транзитивные зависимости.

А также для `uv`:
`uvx / uv tool run` - запуск инструментов (например `ruff`)
`uv sync` - установить зависимости из pyproject.toml
`uv lock` - зафиксировать версии библиотек в файле uv.lock
`uv tree` - вывести дерево зависимостей для текущего проекта
`uv pip install` - установить с помощью pip-а (лучше использовать add, а не эту команду)
Подробнее: [uv Features](https://docs.astral.sh/uv/getting-started/features/#scripts)
...
```
Подробнее про функции первого порядка: [Python’s Functions Are First-Class](https://dbader.org/blog/python-first-class-functions)
...
- `pathlib` — предоставляет классы для удобной работы с путями к файлам и директориям. Рекомендуемой способ общения с файлами на сегодняшний день.
...
[А вот мой сайт uproger, где еще больше вопросов с собеседованйи. ](https://uproger.com/bolee-100-voprosov-s-sobesedovaniya-python-razbor-realnyh-voprosov/)
